...
GEEModuleUI2 <- function(id) {
  ...
  tagList(
    ...
    actionButton(ns("action"), "Run analysis")
  )
}
...
GEEModuleLinear2 <- function(input, output, session, data, data_label, data_varStruct = NULL, nfactor.limit = 10, id.gee) {
  ...
  out <- eventReactive(input$action, {
    data.regress <- data()
...
GEEModuleLogistic2 <- function(input, output, session, data, data_label, data_varStruct = NULL, nfactor.limit = 10, id.gee) {
