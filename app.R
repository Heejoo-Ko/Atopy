                 ...
                 tabPanel("Individual prediction",
                          sidebarLayout(
                            sidebarPanel(
                              ...
                              radioButtons("Method_pred", "Method", c("ARIMA", "Prophet", "Neural network"), "ARIMA", inline = T),
...
server <- function(input, output, session) {
  ...
  obj.ind <- eventReactive(input$action_pred, {
    ...
    if (input$Method_pred == "Prophet"){
      data <- dplyr::filter(tibble(zz_fg), ID == input$ID_pred) %>%
        dplyr::mutate(ds=date, y=get("input$Dep_pred"), cap = 24, floor = 0)
      cut <- round(nrow(data)*0.7)
      train <- data[1:(cut-1),]
      test <- data[cut:nrow(data),]
      
      vars.pred <- setdiff(input$Indep_pred, c("season", "dow"))
      # train2 <- train %>% dplyr::select(c(date, ds, y, input$Dep_pred, vars.pred)) %>% na.omit()
      ...
      fit <- mod %>% fit.prophet(train)
    }
    if (input$Method_pred == "Neural network"){
      data <- dplyr::filter(zz_fg, ID == input$ID_pred)
      cut <- round(nrow(data)*0.7)
      train <- data[1:(cut-1),]
      test <- data[cut:nrow(data),]
      
      xreg <- model.matrix(as.formula(paste("~", input$Indep_pred, collapse = "+")), data = train)[, -1]
      fit <- nnetar(train[[input$Dep_pred]], xreg = xreg)
      # fit <- nnetar(train$sum_score, xreg = train[,13:14])
    }
  ...
  obj.predtable <- reactive({
    if (input$Method_pred == "ARIMA"){
      ...
      a <- forecast(fit, new_data=data[cut:nrow(data),])
      b <- fitted(fit)
      
      yhat <- c(b$.fitted, a$.mean)
      res.ARIMA <- data.table(dplyr::bind_cols(date = data$date, yhat = yhat, sum_score = data[[input$Dep_pred]]))
      
      RMSE <- res.ARIMA[cut:nrow(data),] %>% dplyr::summarize(rmse = mean((yhat-sum_score)^2, na.rm=T)) %>% round(., 3)
      
      table <- datatable(res.ARIMA, rownames=F, extensions = "Buttons", caption = paste0("RMSE for test data = ", RMSE),
      ...
      tab.prophet <- data.table(dplyr::bind_cols(date = ymd(pred$ds), predict = pred$yhat, sum_score = data[[input$Dep_pred]])) %>%
        merge(data.table(dplyr::filter(tibble(zz_fg), ID == input$ID_pred))[, .(date)], by = "date", all.y = T) %>% .[order(date)] %>% 
        as_tsibble() %>% fill_gaps() %>% as.data.table()
      
      RMSE <- tab.prophet[cut:nrow(data), ] %>% dplyr::summarize(rmse = mean((predict-sum_score)^2, na.rm=T)) %>% round(., 3)
      
      table <- datatable(tab.prophet, rownames=F, extensions = "Buttons", caption = paste0("RMSE for test = ", RMSE$rmse),
                         options = c(jstable::opt.data(input$ID_pred),
    ...
    else if (input$Method_pred == "Neural network"){
      data <- dplyr::filter(zz_fg, ID == input$ID_pred)
      cut <- round(nrow(data)*0.7)
      test <- data[cut:nrow(data), ]
      xreg <- model.matrix(as.formula(paste("~", input$Indep_pred, collapse = "+")), data = test)[, -1]
      
      fit <- obj.ind()
      
      a <- forecast(fit, xreg = xreg)
      b <- fitted(fit)
      
      yhat <- c(b, a$mean)
      tab.NNA <- data.table(dplyr::bind_cols(date = data$date, yhat = yhat, sum_score = data[[input$Dep_pred]]))
      RMSE <- tab.NNA[cut:nrow(data), ] %>% dplyr::summarize(rmse = mean((yhat-sum_score)^2, na.rm=T)) %>% round(., 3)
      
      table <- datatable(tab.NNA, rownames=F, extensions = "Buttons", caption = paste0("RMSE for test = ", RMSE$rmse),
                         options = c(jstable::opt.data(input$ID_pred),
  ...
  obj.predplot <- reactive({
    if (input$Method_pred == "ARIMA"){
      ...
      yhat <- c(b$.fitted, a$.mean)
      res.ARIMA <- data.table(dplyr::bind_cols(date = data$date, yhat = yhat, sum_score = data[[input$Dep_pred]])) %>%
        melt(idvars = "ID", measure.vars = c("yhat", "sum_score"))
      
      ggpubr::ggline(res.ARIMA, "date", "value", color = "variable", plot_type  = "l", xlab = "", ylab = input$Dep_pred) +
        geom_vline(xintercept=res.ARIMA[cut,]$date, linetype='dotted')
      ...
      tab.prophet <- data.table(dplyr::bind_cols(date = as.Date(pred$ds), predict = pred$yhat, sum_score = data[[input$Dep_pred]])) %>% 
        merge(data.table(dplyr::filter(tibble(zz_fg), ID == input$ID_pred))[, .(date)], by = "date", all.y = T) %>% 
        as_tsibble() %>% fill_gaps() %>% as.data.table() %>%
        melt(measure.vars = c("predict", "sum_score"))
      
      ggpubr::ggline(tab.prophet, "date", "value", color = "variable", plot_type  = "l", xlab = "", ylab = input$Dep_pred) +
        geom_vline(xintercept=tab.prophet[cut,]$date, linetype='dotted')
    } else if (input$Method_pred == "Neural network"){
      data <- dplyr::filter(zz_fg, ID == input$ID_pred)
      cut <- round(nrow(data)*0.7)
      test <- data[cut:nrow(data), ]
      ...
      ggpubr::ggline(tab.NNA, "date", "value", color = "variable", plot_type  = "l", xlab = "", ylab = input$Dep_pred) +
        geom_vline(xintercept=tab.NNA[cut,]$date, linetype='dotted')
    }
